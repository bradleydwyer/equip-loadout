...
async fn test_majority_vote_with_agreement() {
    let candidates = vec![
        ...
        Candidate::new("Paris is the capital of France"),
        Candidate::new("Lyon is the capital of France"),
    ];
...
async fn test_weighted_vote_with_confidence() {
    let candidates = vec![
        Candidate::new("Rust is fast").with_confidence(0.95).with_model("expert-model"),
        Candidate::new("Rust is slow and terrible").with_confidence(0.1).with_model("bad-model"),
        Candidate::new("Rust is fast and safe").with_confidence(0.85).with_model("good-model"),
    ];

    let result = consensus(&candidates, "weighted_vote", None).await.unwrap();
    assert!(result.agreement_score > 0.5);
    // The high-confidence candidates should win
